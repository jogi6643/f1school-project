@extends(Auth::user()->role==1? 'layouts.admin':'layouts.trainer')
 @section('content')
            <div class="content-heading">
               @if($planname!='NO')
                   <h2>{{$k}}
             :{{$planname}}:Assign Date</h2>
...
function goBack() {
  ...
  <tbody>
    ...
    <input  type="date" name="data_{{ $schoolid}}_{{ $planidd}}_{{$acyear}}_{{$Course1->id}}_{{$Course1->doctypeid}}_{{$Course1->coursemaster_id}}"  value="@if(isset($ass[$Course1->id])){{$ass[$Course1->id]}}@endif"  />
