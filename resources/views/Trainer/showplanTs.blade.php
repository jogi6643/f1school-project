...
function goBack() {
  window.history.back();
   ...
   <form hidden="" method="post" action="{{url('storeschoolplanmasterbytr')}}" enctype="multipart/form-data">
            @csrf
   ...
   <div class="col-md-12">
    ...
    <table class="table table-striped table-bordered" id="planlistid">
 	...
 	<tbody>
		...
		<tr>
		 <td>{{$i}}</td>
		 <td><a href="{{url('viewcourseT')}}/{{$plansch}}" class="btn btn-warning">{{$planperschool1->name}}</a></td> 
         <td>{{$planperschool1->year}}</td>
          <td><a href="{{url('viewstudentlist')}}/{{$plansch}}" class="btn btn-warning">{{$planperschool1->counts}}</a></td>
         <td>{{date('d/m/y i:h:s',strtotime($planperschool1->created_at))}} </td>

         <td >
          <a href="{{url('viewcourseinPlan')}}/{{$plansch}}" class="btn btn-warning">View Course
          </a>
         </td>
 ...
 <script type="text/javascript">
 	$(document).ready(function(){
      ...
      var v=$('#academicyear').val();
         ...
         $.ajax({
             url:"{{url('schoolplanfetchacademicyear_admin')}}",
