...
@section('content')
 ...
 <table class="table table-striped table-bordered" id="schollistid">
  ...
  @foreach($info as $k)
  <?php $idscr=$trainerid."_".$k['id'];?>
    <tr>
      <td>{{$i}}</td> 
      <td> <a href="{{url('trschooinfo')}}/{{base64_encode($k['id'])}}">{{$k['schoolname']}}</a></td>
      <td>{{$k['locationname']}}</td>
      <td>{{$k['cityname']}}</td>
      <td>{{$k['cityname']}}</td>
      <td><a href="{{url('showliststudent')}}/{{base64_encode($k['id'])}}"> Student List</td>
              <td>
               <a href="{{url('createteambytr')}}/{{base64_encode($idscr)}}"><button type="button" class="btn btn-success">Create Team</button></a>
             </td>

                 <td>
               <a href="{{url('viewTeamtr')}}/{{base64_encode($idscr)}}"><button type="button" class="btn btn-success">View Team</button></a>
             </td>
