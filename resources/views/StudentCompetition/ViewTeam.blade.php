 ...
 <table id="example" class="table table table-striped">
    ...
    <tbody>
     ...
     <?php $ids=$cts->id."_".$cts->school_id."_".$cts->student_id;?>
        ...
        <td>{{$cts->team_Name}}</td>
       <?php $y=explode("_",$cts->student_id)?>
       @if($y[1]=='student')
        <td>{{$studentname}}</td>
        @else
        <td>{{$y[1]}}</td>
        ...
        <td><a href="{{url('assignmember')}}/{{base64_encode($ids)}}"><button class="btn btn-info">View Team Member</button></a></td>
        
        <td>
          <a href="{{url('editteam')}}/{{base64_encode($ids)}}"><button class="btn btn-success">edit</button></a>
<!-- <a href="{{url('deleteteam')}}/{{base64_encode($ids)}}" class="btn btn-danger disabled" >delete</a> -->
        </td>
      </tr>
      @endforeach
      @else
