...
<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.4.1/jquery.min.js"></script>
             ...
             <div class="content-heading">
               <div>{{$comp->Competition_name}}|{{$comp->academicyear}}|Edit Awards
               ...
               <div class="ml-auto">
                  <div class="btn-group">
                     <a href="{{url('add-awards')}}/{{$compid}}" class="btn btn-warning">Add</a>
                  </div>
               </div>
			   @endif
			   @endif
...
<div class="col-md-12">
 ...
 <form method="post" action="{{url('editawardsstore')}}" enctype="multipart/form-data">
   @csrf
   <div class="form-group">
     <input hidden=""  class="form-control" name="compid" value="{{$compid}}">
     <input hidden="" type="text" class="form-control" name="awardsid" value="{{$awards}}">
   </div>
    <div class="form-group">
   ...
   <div class="form-group div" id="addcontentawards">
    ...
    <div class="col-md-4">
    <input type="text" required="" value="{{$awardname}}" class="form-control" name="awards[]" placeholder="Award Name..">
    </div>
     <textarea class="form-control" value="{{$awardcontent}}" id="summernote" placeholder="awards content ..." rows="5" name="awrdscontent[]">{{$awardcontent}}</textarea>
               ...
               <select required="" name="school[]" class="form-control" id='udpcityid' multiple="multiple">
                  @if(count($schoolv)>0)
                     @foreach($schoolv as $lc) 
                        @if($lc->school_id==old('school_id')||in_array($lc->school_id,$schoolid2))
 ...
 <script type="text/javascript">
   $(document).ready(function() {
     ...
     $('#udpcitylist1').multiselect({
      ...
      //    var nurl="{{url('getnominateteams')}}/"+btoa(v)+"/"+btoa(c);
      //    alert(nurl);
      ...
      // });
         ...
         var v=$('#udpcityid').val();
         var c = "<?php echo $compid ?>";
          var awardid = "<?php echo $awards ?>";
         var nurl="{{url('getnominateteams')}}/"+btoa(v)+"/"+btoa(c)+"/"+awardid;
          $.get(nurl,function(data){
            $('#udpcitylist').html(data);
