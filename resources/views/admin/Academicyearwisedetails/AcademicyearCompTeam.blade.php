@extends(Auth::user()->role==2? 'layouts.coadmin':'layouts.admin')
 ...
 <div>
  <h3 class="text-left">School Name:{{$compname}}</h3>
...
function goBack() {
  ...
  ?>
     ...
     <td><a class="btn btn-info" href="{{ url('academicYear-team-in-competition-School'.'/'.base64_encode($ids)) }}">{{$row->teamcount}}</a></td> 
		</tr>
  @endforeach  
