...
.white {
  ...
  <form action="{{url('sbmitparticipant')}}" method="post">
 @csrf
            ...
            @endif
              <select required="" class="form-control" name="year"  id="year" onchange="membership()">
               ...
               <?php for ($year=2015; $year <= date('Y')+1; $year++): 
                  $check1 = ($year-1)."-".($year);
                  $check = session()->get('Year');
                ?>
                <option @if($check==$check1) selected="selected" @endif value="{{$check1}}">{{$check1}}</option>
                <?php endfor; ?>
                @else
