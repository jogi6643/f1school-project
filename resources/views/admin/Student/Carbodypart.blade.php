        ...
        <tr id="row1">
         <td class="text-center">{{$j}}</td>
           <td> 
            ...
            <option value="{{$bodypart1->id}}">{{$bodypart1->parts}}</option>
            @endforeach
            @else
            <option>Assign Price Plan First..</option>
            @endif
...
<div class="container">
 <div class="jumbotron">
    <h1 class="text-center">Car Body Part</h1>
    <h2 class="text-center">No Assign Price Plan</h2>
  </div>
</div>
@endif
...
{
	html+='<option  value="{{$bodypart->id}}">{{$bodypart->parts}}</option>';
}
@endforeach
html+="</select></td>";
