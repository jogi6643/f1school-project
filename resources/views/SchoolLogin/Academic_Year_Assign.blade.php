 ...
 @section('content')
            ...
            @endif
   <div class="col-md-12">
         <form method="post" action="{{url('check-data-according-to-year-wise-school')}}">
            @csrf
        <div cass="col-md-12">
    ...
    @for ($year = 2015; $year <= $year1; $year++)
      {{ $y= ($year-1)."-".$year}}
   <option @if($y==$ac_year) selected="selected" @endif  value="<?=($year-1)."-".($year);?>"><?=($year-1)."-".$year;?></option>
    @endfor
</select>
           </div>
       </div>
...
<div hidden="" class="col-md-12">
  ...
  <?php $teamidschoolid = $row->id."_".$row->school_id ?>
     <td><a target="__blank" href="{{ url('viewteambyschool'.'/'.base64_encode($teamidschoolid)) }}">{{$row->team_Name}}</a></td>
...
<div class="col-md-12">
  ...
  <tbody>
    ...
    @foreach($eq as $k)
   <?php $info = $k->plan."_".$k->schoolid."_".$k->year; 
   ...
   ?>
    <tr>
      ...
      <td><a href="{{url('available-student-in-plan-school'.'/'.base64_encode($info)) }}" target="__blank">{{$k->planname}}</a></td>
